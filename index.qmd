...
html`
			<div>
				...
				${m.default_figure != ""  ? html `<div><img class="blm" src="figures/timeseries_images/${m.default_figure}"></img></div>` : html `<div>No image found for this indicator</div>`}
</div>`
...
function check_csvurl(base,m){
  const cciea_url = base + m.cciea_filename;
  const pi_url = base + m.PI_filename;
  
  return fetch(cciea_url, { method: 'HEAD'})
    ...
    .catch(error => {
      console.log("Fetch failed, defaulting to PI:", error.message);
      return pi_url
  });
...
csvurl = {
  const cciea_url = raw_base + m.cciea_filename;
  const pi_url = raw_base + m.PI_filename;

  return fetch(cciea_url, { method: 'HEAD' })
    .then(res => res.ok ? cciea_url : pi_url)
    .catch(() => pi_url);
}
...
html`
			<div>
				...
				<div>
				${m.region!="" ? html `<span class="bold">Region: </span><span>${m.region}</span>` :  ``}
				${m.latitude!="NA" ? html `<span class="bold">Latitude: </span><span>${m.latitude} N</span>` : ``}
				${m.latitude2!="NA" ? html `<span class="bold"> - </span><span>${m.latitude2} N</span>` : ``}
				${m.longitude!="NA" ? html `<span class="bold">Longitude: </span><span>${m.longitude} E</span>` : ``}
				${m.longitude2!="NA" ? html `<span class="bold"> - </span><span>${m.longitude2}</span>` : ``}
				</div>
				<div>
				...
				<div>
				${m.year_begin!="NA" ? html `<span class="bold">Date range: </span><span>${year(m.year_begin)} - ${year(m.year_end)}</span>` : ``}
				</div>
				<div>
				${m.min!="NA" ? html `<span class="bold">Indicator range: </span><span>min: ${m.min} max: ${m.max}</span>` : ``}
				</div>
				<div>
				<span class="bold">Serve this data for CCIEA? </span><span>${yesno(m.serve_flag)}</span><br><span class="bold">Whale Entanglement dataset? </span><span>${yesno(m.whale_entanglement)}</span><br><span class="bold">Salmon web page? </span><span>${yesno(m.Salmon_Web)}</span>
				</div>
				<div>
				${m.no_download==1 ? html `<span class="bold">No download, private data</span>`
					: html `<span class="bold">Download data:</span>
						&nbsp;&nbsp;<span><a class="noline" href='${csvurl}' target='_blank'>csv file</a></span>
					`}
				</div>
				<div>

...
html`
			<hr><div>
				<p><div><span>Metadata for <span class="bold">${m.Title}</span>:</span></div></p>
...
```{ojs}
//| echo: false
html`<hr><h3>View and/or edit ${last_year} metadata and data for:  ${pis[indx].name}</h3>
...
html`<hr><h3>Upload ${pis[indx].name} ${esr_year} data and metadata</h3>`
...
html`<h5>We will check for the following:</h5>`
...
```{ojs}
//| echo: false
html`<br><p><h5>3. If your files all pass the check, go the Google Drive folder for uploading your data:</h5></p>`
...
*  Select each indicator on the left to see the currentindicator metadata
